import Link from "next/link";
import { ChevronLeft } from "lucide-react";
import { ConsentService } from "@/lib/services/consent.service";
import { ConsentFormEditForm } from "./ConsentFormEditForm";
import { SignaturesTable } from "./SignaturesTable";

export default async function ConsentFormDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const [form, { items: signatures, total }] = await Promise.all([
    ConsentService.getById(id),
    ConsentService.getSignatures(id, { page: 1, limit: 20, dir: "desc" }),
  ]);

  return (
    <>
      <div className="sticky top-0 z-10 bg-surface border-b border-border px-6 py-4">
        <div className="flex items-center gap-3 max-w-3xl mx-auto">
          <Link
            href="/admin/consent-forms"
            className="flex items-center gap-1 text-sm text-ink-muted hover:text-ink transition-colors duration-80"
          >
            <ChevronLeft className="w-4 h-4" aria-hidden />
            Consent Forms
          </Link>
          <span className="text-ink-subtle">/</span>
          <h1 className="font-display text-lg font-semibold text-ink truncate">{form.title}</h1>
        </div>
      </div>

      <div className="px-6 py-6 max-w-3xl mx-auto space-y-6">
        <ConsentFormEditForm form={form} />

        <div className="bg-surface-50 border border-border rounded-card overflow-hidden" style={{ boxShadow: "var(--shadow-card)" }}>
          <div className="px-5 py-4 border-b border-border flex items-center justify-between">
            <h2 className="font-display text-base font-semibold text-ink">Signatures</h2>
            <span className="text-xs text-ink-muted">{total} total</span>
          </div>
          <SignaturesTable signatures={signatures} />
        </div>
      </div>
    </>
  );
}
